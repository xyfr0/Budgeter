        ...
        try (Connection conn = Koneksi.connect()) {
            while (true) {
                try (PreparedStatement ps = conn.prepareStatement(SQL_QUERY
                        + "SELECT COUNT(*) FROM TransactionHistory WHERE TransactionID = ?")) {
                    ...
                    if (count == 0) {
                        try (PreparedStatement insertPs = conn.prepareStatement(SQL_QUERY
                                + "INSERT INTO TransactionHistory (TransactionID, Amount, TransactionDate, TransactionType, Description) "
                                + "VALUES (?, ?, ?, ?, ?);")) {
                            insertPs.setInt(1, transaction.getId());
                            insertPs.setDouble(2, transaction.getAmount());
                            insertPs.setObject(3, transaction.getDate());
                            insertPs.setString(4, transaction.getTransactionType());
                            insertPs.setString(5, transaction.getDescription());
